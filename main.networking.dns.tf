...
data "azapi_resource_action" "avs_default_dns" {
  ...
  method                 = "GET"
  resource_id            = "${azapi_resource.this_private_cloud.id}/workloadNetworks/default/dnsZones"
  type                   = "Microsoft.AVS/privateClouds/workloadNetworks/dnsZones@2023-09-01"
  response_export_values = ["*"]
...
resource "azapi_update_resource" "dns_default_service_ips" {
  ...
  name      = data.azapi_resource_action.avs_default_dns["default"].output.value[0].name
  parent_id = "${azapi_resource.this_private_cloud.id}/workloadNetworks/default"
  type      = "Microsoft.AVS/privateClouds/workloadNetworks/dnsZones@2023-09-01"
  body = {
    properties = {
...
resource "azapi_resource_action" "dns_default_zone" {
  ...
  method      = "PATCH"
  resource_id = data.azapi_resource_action.avs_default_dns["default"].output.value[0].id
  type        = "Microsoft.AVS/privateClouds/workloadNetworks/dnsZones@2023-09-01"
  body = {
    properties = {
      displayName  = data.azapi_resource_action.avs_default_dns["default"].output.value[0].properties.displayName
...
resource "azapi_resource_action" "dns_zone_destroy_non_empty_start" {
  for_each = length(var.default_dns_ips) > 0 ? { "default" : var.default_dns_ips } : {}
  ...
  body = {
    properties = {
      displayName  = data.azapi_resource_action.avs_default_dns["default"].output.value[0].properties.displayName
      dnsServerIps = data.azapi_resource_action.avs_default_dns["default"].output.value[0].properties.dnsServerIps
      domain       = []
      dnsServices  = 1
    }
  }
